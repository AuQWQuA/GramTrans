...
def deparse_ll(node, indent=0, hasindent=True, newline=True):
    ...
    if node.type in ["block", "class_body", "switch_block"]:
        returnlist.append(" " + "{\n")
        for child in node.children[1:-1]:
            returnlist += deparse_ll(child, indent + 1)
        returnlist.append("}")
        # returnlist.append(" " * indent * 4 + "}")
        returnlist.append("\n")
    elif node.type == "for_statement":
        for i in range(node.child_count - 1):
            if i == 2:
                returnlist += deparse_ll(node.children[i], indent, False, False)
            else:
                returnlist += deparse_ll(node.children[i], indent)
        returnlist += add_statement(node.children[-1], indent)
    elif node.type == "enhanced_for_statement":
        returnlist.append("enhanced_for ")
        for child in node.children[1:-1]:
            returnlist += deparse_ll(child, indent)
        returnlist += add_statement(node.children[-1], indent)
    elif node.type == "if_statement":
        for i in range(2):
            returnlist += deparse_ll(node.children[i], indent)
        ...
        if node.child_count > 3:
            ...
            if node.children[4].type == "if_statement":
                returnlist.append(" ")
                returnlist += deparse_ll(node.children[4], indent, False)
            else:
                returnlist += add_statement(node.children[4], indent)
    elif node.type == "while_statement":
        for i in range(2):
        ...
        returnlist.append(node.children[1].text.decode("utf8") + " ")
        returnlist += deparse_ll(node.children[0], indent)
        returnlist.append(" ")
        returnlist += deparse_ll(node.children[2], indent)
    elif node.type in ['unary_expression']:
        if node.children[0].type == "-":
            returnlist.append("NEG ")
        else:
            returnlist += deparse_ll(node.children[0], indent)
        returnlist += deparse_ll(node.children[1], indent)
        ...
        for i in range(2):
            returnlist += deparse_ll(node.children[i], indent)
        returnlist += deparse_ll(node.children[2], indent, False)
    elif node.type == "switch_block_statement_group":
        for i in range(2):
            returnlist += deparse_ll(node.children[i], indent)
        if node.child_count > 2:
...
def ll_deparse(node, indent=0, hasindent=True, newline=True):
    ...
    if node.type in ["block", "class_body", "switch_block"]:
        returnlist.append(" " + "{\n")
        for child in node.children[1:-1]:
            returnlist += ll_deparse(child, indent + 1)
        returnlist.append(" " * indent * 4 + "}")
        returnlist.append("\n")
    elif node.type in ["method_invocation"]:
        for child in node.children[1:]:
            returnlist += ll_deparse(child, indent)
    elif node.type == "for_statement":
        for i in range(node.child_count - 1):
            if i == 2:
                returnlist += ll_deparse(node.children[i], indent, False, False)
            else:
                returnlist += ll_deparse(node.children[i], indent)
        returnlist += add_statement(node.children[-1], indent)
    elif node.type == "enhanced_for_statement":
        ...
        if node.child_count > 3:
            ...
            if node.children[4].type == "if_statement":
                returnlist.append(" ")
                returnlist += ll_deparse(node.children[4], indent, False)
            else:
                returnlist += add_statement(node.children[4], indent)
    elif node.type == "while_statement":
        for i in range(2):
        ...
        returnlist += ll_deparse(node.children[1], indent)
        returnlist.append(" " + node.children[0].text.decode("utf8") + " ")
        returnlist += ll_deparse(node.children[2], indent)
    elif node.type in ['unary_expression']:
        if node.children[0].type == "NEG":
            returnlist.append("-")
        else:
            returnlist += ll_deparse(node.children[0], indent)
        returnlist += ll_deparse(node.children[1], indent)
        ...
        for i in range(2):
            returnlist += ll_deparse(node.children[i], indent)
        returnlist += ll_deparse(node.children[2], indent, False)
    elif node.type == "switch_block_statement_group":
        for i in range(2):
            returnlist += ll_deparse(node.children[i], indent)
        if node.child_count > 2:
