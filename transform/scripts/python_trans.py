    ...
    if node.type in ["block"]:
        ...
        while i < node.child_count:
            if node.child(i).type == "comment" or node.child(i).type == "line_continuation":
                i += 1
                continue
            returnlist += [deparse(node.child(i), indent + 1)]
        ...
        while i < node.child_count:
            if node.child(i).type == "comment" or node.child(i).type == "line_continuation":
                i += 1
                continue
            returnlist += [deparse(node.child(i), indent)]
            if node.child(i).type in simple_statement:
                if i + 1 < node.child_count and node.child(i + 1).type != ";":
                    returnlist += ["\n"]
                elif i + 1 == node.child_count:
        ...
        returnlist = [deparse(node.child(i), indent) for i in range(node.child_count) if node.child(i).type != 'comment' and node.child(i).type != 'line_continuation']
