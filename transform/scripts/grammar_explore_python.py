...
def get_used_rules(node):
    ...
    children = [child for child in node.children if child.type not in ["comment", "line_continuation"]]
                ...
                if len(tempsuccess) == 2 and len(tempsuccess[0][1]) > len(tempsuccess[1][1]):
                    ...
                    if tempsuccess[0][0]['type'] == "SYMBOL":
                        ...
                        for temp in tempsuccess:
                            print(temp)
                        assert "Ambiguous choice", tempsuccess
                else:
                    print(subpart)
                    print("Ambiguous choice")
                    for temp in tempsuccess:
            ...
            if len(matchparts) > 0:
                ...
                for i in range(1, len(matchparts)):
                    ...
                    if not merge_success:
                        assert "Cannot merge repeat parts", matchparts
                return True, matchpart, matchchildren, matchsymbol, matchnewrules
            else:
                return True, matchpart, matchchildren, matchsymbol, matchnewrules
    ...
    if matchchildren != children:
        assert "Not all children matched", [matchchildren, children]
    ...
    if success:
        if node.type in used_rules:
            ...
            if not issuccess:
                ...
                print(value)
                assert "Cannot merge rules", node.type
            used_rules[node.type] = mergepart
        else:
            used_rules[node.type] = matchpart
    else:
        assert "Cannot match rule", node.type
    ...
    for child in matchsymbol:
        if child.type not in ['identifier', 'integer', 'float', 'string_content'] and child.type not in external_symbols:
...
def convert_node(node):
    ...
    if node_type == "SYMBOL":
    ...
    elif node_type in ("PREC_LEFT", "PREC_RIGHT", "PREC"):
        ...
        return f"alias({named}, {convert_node(node['content'])})"
